# Проект 3. Рейтинг отеля по данным сайта Booking. EDA + Feature Engineering. Соревнование на Kaggle
<table>
...
- Загружаем данные и объединяем трейн и тест в один датасет для корректной обработки признаков;
- Изучаем датасет на предмет наличия пропусков и типов данных;
- Обработка данных: извлечение данных из выборки, создание новых признаков;
- Очистка и отбор признаков: избавляемся от признаков, на которых модель обучаться не будет, отбираем признаки на основании мультиколлинеарности и значимости признаков по 2 методикам - раздельно для категориальных и непрерывных признаков;
- Готовим данные для модели: выделяем тестовую часть, разделяем тренинговый датасет: 80% на обучение, 20% на валидацию, проверяем размерности получившихся датасетов;
- Построение и обучение модели;
- Сравниваем предсказанные значения с реальными, и смотрим насколько они в среднем отличаются. В качестве метрики используем Mean Absolute Error (MAE) и Mean Absolute Percentage Error (MAPE);
- Проверка соответствия написанного кода стандарту PEP8;
- Оформление проекта;
- Загружаем код на площадку Kaggle для участия в соревновании и дублируем на GitHub.
...
Несмотря на оценки вычисленные в ноутбуке, в лидерборде показывает другую оценку - 11.85073.
